...
class ProcessingOrderList extends StatefulWidget {
  @override
  _ProcessingOrderListState createState() => _ProcessingOrderListState();
}

class _ProcessingOrderListState extends State<ProcessingOrderList> {
  ...
  _getUserInfo();
    ...
    var res = await CallApi().getData('/api/showOrdersType/Processing');
    ...
    if (res.statusCode == 200) {
      ...
      store.dispatch(ProcessingOrderListAction(orderList.reversed.toList()));
    ...
    store.dispatch(ProcessingOrderLoadingAction(false));
  }
  ...
  Widget build(BuildContext context) {
       return  StoreConnector<AppState, AppState>(
        ...
        body: 
        store.state.processingOrderLoading
            ? Center(
                child: CircularProgressIndicator(),
              )
            : 
            store.state.processingOrderList.length == 0
                ? Center(
                    child: Column(
  ...
  List<Widget> _showOrdersList() {
    List<Widget> list = [];
    // int checkIndex=0;
    for (var d in store.state.processingOrderList) {
