...
class DeliveredOrderList extends StatefulWidget {
  @override
  _DeliveredOrderListState createState() => _DeliveredOrderListState();
}
...
class _DeliveredOrderListState extends State<DeliveredOrderList> {
  ...
  _getUserInfo();
    ...
    var res = await CallApi().getData('/api/showOrdersType/Delivered');
    ...
    if (res.statusCode == 200) {
      ...
      store.dispatch(DeliveredOrderListAction(orderList.reversed.toList()));
    ...
    store.dispatch(DeliveredOrderLoadingAction(false));
  }
  ...
  Widget build(BuildContext context) {
       return  StoreConnector<AppState, AppState>(
        ...
        body: 
        store.state.deliveredOrderLoading
            ? Center(
                child: CircularProgressIndicator(),
              )
            : 
            store.state.deliveredOrderList.length == 0
                ? Center(
                    child: Column(
  ...
  List<Widget> _showOrdersList() {
    List<Widget> list = [];
    // int checkIndex=0;
    for (var d in store.state.deliveredOrderList) {
