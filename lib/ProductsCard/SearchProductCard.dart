...
class SearchProductsCard extends StatefulWidget {
  final user;
  final index;
  SearchProductsCard(this.user, this.index);
  @override
  _SearchProductsCardState createState() => _SearchProductsCardState();
}

class _SearchProductsCardState extends State<SearchProductsCard> {
      ...
      child: GestureDetector(
        onTap: () {
          Navigator.push(
              context,
              SlideLeftRoute(
                  page: ProductsDetailsPage(widget.index)));
        },
        child: Container(
                      ...
                      child: Stack(
                        children: <Widget>[
                          ...
                          Positioned(
                            ...
                            child:  GestureDetector(
                                              onTap: () {
                                                _editDeleteBottomSheet(store.state.searchProductList[widget.index]);
                                              },
                        ...
                        Center(
                          child: Container(
                            ...
                            child: 
                            store.state.searchProductList[widget.index]['productImage']==null ||
                            store.state.searchProductList[widget.index]['productImage']==""?
                            Image.asset(
                            ...
                            CachedNetworkImage(
                                            imageUrl: CallApi().getUrl()+store.state.searchProductList[widget.index]['productImage'],
                        ...
                        ////// <<<<< New tag start >>>>> //////
                     store.state.searchProductList[widget.index]['discount']==0 ||
                     store.state.searchProductList[widget.index]['discount']==null?
                     Container():
                        Container(
                          ...
                          child: Row(
                            ...
                            children: <Widget>[
                              Container(
                                  ...
                                  child: Text(
                                    productDiscount != 0
                                        ? "${store.state.searchProductList[widget.index]['discount']}% "+"Off"
                                        : "",
                                    style: TextStyle(
                      ...
                      children: <Widget>[
                        ...
                        Expanded(
                          ...
                          //  alignment: Alignment.center,
                            child: Text(store.state.searchProductList[widget.index]['productName']==null?"":
                            store.state.searchProductList[widget.index]['productName'],
                                maxLines: 1,
                            ...
                            child: Row(
                              ...
                              children: <Widget>[
                                Center(
                                  ...
                                  //  alignment: Alignment.center,
                                    child: Text(
                                         store.state.searchProductList[widget.index]['discountPrice']==null?"0.00 Tk":
                                         "${store.state.searchProductList[widget.index]['discountPrice']} Tk",
                                      overflow: TextOverflow.ellipsis,
                                      style: TextStyle(
                                          fontSize: 16,
                                          color: appColor,
                             ...
                             store.state.searchProductList[widget.index]['discount']==null
                                    ...
                                    : Expanded(
                                        child: Container(
                                          ...
                                          child: Text(
                                            "${store.state.searchProductList[widget.index]['price']} Tk",
                                            overflow: TextOverflow.ellipsis,
                                            style: TextStyle(
                                                fontSize: 12,
                                                color: Colors.grey,
  ...
  void _editDeleteBottomSheet(d) {
   ...
   Future<Null> _showDeleteDialog(d) async {
    ...
    if (res.statusCode == 200) {
        ...
        store.state.searchProductList.remove(d);
     ...
     if(store.state.productList.length>0){
       ...
       store.dispatch(ProductListAction(store.state.productList));
          ...
          if(store.state.emptyStockProductList.length>0){
           for (var data in store.state.emptyStockProductList) {
        if (data['id'] == d['id']) {
           store.state.emptyStockProductList.remove(data);
          break;
        }
      }
       ...
       store.dispatch(EmptyStockProductListAction(store.state.emptyStockProductList));
          ...
          if(store.state.availableProductList.length>0){
           for (var data in store.state.availableProductList) {
        if (data['id'] == d['id']) {
           store.state.availableProductList.remove(data);
          break;
        }
      }
