...
class _AllOrderListState extends State<AllOrderList> {
  ...
  _getUserInfo();
    ...
    var res = await CallApi().getData('/api/showOrders');
    ...
    if (res.statusCode == 200) {
      ...
      store.dispatch(OrderListAction(orderList.reversed.toList()));
  ...
  Widget build(BuildContext context) {
       return  StoreConnector<AppState, AppState>(
        ...
        body: 
        store.state.orderLoading
            ? Center(
                child: CircularProgressIndicator(),
              )
            : 
            store.state.orderList.length == 0
                ? Center(
                    child: Column(
